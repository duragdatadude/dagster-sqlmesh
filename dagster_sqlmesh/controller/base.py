...
class SQLMeshInstance:
    ...
    def plan_and_run(
        ...
        if plan_and_run_options is not None:
            run_options = t.cast(
                RunOptions, {**run_options, **plan_and_run_options.get("run", {})}
            )
            plan_options = t.cast(
                PlanOptions, {**plan_options, **plan_and_run_options.get("plan", {})}
            )
            shared = plan_and_run_options.get("shared", {})
            run_options = t.cast(RunOptions, {**run_options, **shared})
            plan_options = t.cast(PlanOptions, {**plan_options, **shared})
